...
def remove_pixel_augmentation (data: ArrayLike, size) -> ArrayLike:
    ...
    return create_new_tuple_from_augment(data, new_training_pictures = new_training_pictures, new_test_pictures = new_test_pictures)

def create_new_tuple_from_augment (data: ArrayLike, new_training_pictures: ArrayLike, new_test_pictures: ArrayLike):
    training_data_tuple = ( np.concatenate((data[0][0], new_training_pictures), axis=0), np.concatenate((data[0][1], data[0][1]), axis=None))
    test_data_tuple = ( np.concatenate((data[1][0], new_test_pictures), axis=0), np.concatenate((data[1][1], data[1][1]), axis=None))
