...
def load_mnist_all(path, file_names, number_of_samples):
    ...
    if number_of_samples < 10000:
        testing_images = load_mnist_image(path, file_names[2], number_of_samples)
        testing_labels = load_mnist_label(path, file_names[3], number_of_samples)
    else:
...
def normalize_data(data):
    train_data, test_data = data
    normalized_train_data = (round(train_data[0] / 255, 2), train_data[1])
    normalized_test_data = (round(test_data[0] / 255, 2), test_data[1])
