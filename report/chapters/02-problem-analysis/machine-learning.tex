\section{Machine learning}\label{sec:machine-learning}\todo[inline]{This should be the first section of the chapter}
...
The group has also considered the Iris dataset, another introductory dataset, however that dataset might have too few data samples ~\cite{mnist-vs-iris}, which is also why the group has chosen to work with \gls{mnist}. As outlined before, there have been done some sort of preprocessing regarding the \gls{mnist}, which further solidified the choice regarding \gls{mnist} \todo[inline]{Do not look only at the Iris example. Think of other datasets. This section could be moved to methodology and explained in more detail}.


We have in this chapter presented the pipeline, and gave an overview of the different components of the pipeline. With this knowledege we can now move on to the problem statement. \todo[inline]{Add text to describe why linear vs nonlinear would work in this dataset.}
